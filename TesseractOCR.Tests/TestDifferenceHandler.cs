...
namespace Tesseract.Tests
{
    ...
    ///     This is handy when updating the underlying tesseract engine dll's and/or tessdata as the output of a number of tests
    ///     may
    ...
    public class P4MergeTestDifferenceHandler : ITestDifferenceHandler
    {
        public void Execute(string actualResultFilename, string expectedResultFilename)
        {
            ...
            if (File.Exists(expectedResultFilename))
            {
                ...
                {
                    Console.WriteLine(
                        $"Expected results to be \"{expectedResultFilename}\" but was \"{actualResultFilename}\", launching merge tool.");
                    ...
                    expectedResult = TestUtils.NormalizeNewLine(File.ReadAllText(expectedResultFilename));
                    if (expectedResult != actualResult)
                        Assert.Fail("Expected results to be \"{0}\" but was \"{1}\".", expectedResultFilename,
                            actualResultFilename);
                }
        ...
        private static void Merge(string actualResultFileName, string expectedResultFilename)
        {
            // Note currently merge tool is hard coded and expected the command line "merge.exe %base %left %right %merged"
            TestUtils.Cmd("p4merge.exe", expectedResultFilename, actualResultFileName, expectedResultFilename,
